...
const MusicPlayer = ({ isMinimized: initMin = false }) => {
  ...
  useEffect(() => {
    if (audioRef.current) {
      ...
      setProgress(0);
      if (isPlaying) {
        audioRef.current.play().then(startTimer).catch(() => setIsPlaying(false));
      }
    }
  }, [currentTrackIndex]);
      ...
      <div className={`relative flex items-end ${isMinimized ? 'hidden' : 'flex'}`}>
        ...
        <div 
          className={`bg-night border border-white/10 rounded-l-[20px] shadow-2xl p-4 w-[240px] h-[104px] mr-[-20px] pr-[36px] transition-all duration-300 overflow-y-auto flex flex-col gap-1 z-0 ${
            showPlaylist ? 'opacity-100 translate-x-0 pointer-events-auto' : 'opacity-0 translate-x-10 pointer-events-none'
          }`}
        >
          <p className="text-[11px] font-bold text-white/40 uppercase tracking-wider mb-1">Playlist</p>
        ...
        {/* --- EXPANDED PLAYER BOX --- */}
        <div 
          className="gap-4 bg-night border border-white/10 rounded-[20px] shadow-2xl p-4 w-[340px] items-center flex z-10"
        >
          {/* --- ROTATING VINYL DISC --- */}
          <div className="relative flex-shrink-0 w-[72px] h-[72px]">
            ...
            <div className="absolute inset-[4px] rounded-full border border-black/40" style={{ boxShadow: 'inset 0 0 0 2px rgba(255,255,255,0.05)' }} />
            <div className="absolute inset-[12px] rounded-full border border-white/5" />
            
            {/* Center Album Artwork / Disc Wrapper */}
            <div
            ...
            >
              {currentTrackIndex === 1 ? (
                ...
                /* Dynamic Custom Emoji Disc Art for winter festival or extra tracks */
                <span className="text-[20px] select-none filter drop-shadow-[0_2px_4px_rgba(0,0,0,0.3)]">
                  {currentTrack.emoji}
                </span>
              )}

              {/* Inner Label Vinyl shading ring */}
          ...
          {/* Player controls metadata display */}
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-center gap-0.5">
              ...
              {/* Playlist Drawer Controller */}
              <button 
                onClick={() => setShowPlaylist(!showPlaylist)}
                className={`p-1.5 rounded-md transition-colors ${showPlaylist ? 'text-white' : 'text-white/60 hover:text-white hover:bg-white/5'}`}
              >
